...
- **🔬 拆文库**：导入爆款小说 → AI 一次性拆解（概述/黄金三章/逐章摘要/聚合分析/文风分析/爽点/硬伤）→ 逐段编辑 → 大纲和卷纲生成时参考
- **📋 设定库**：左侧栏独立模块，导入小说 → AI 提取角色/世界观/规则/关系 → 逐字段编辑 → 大纲生成时自动参考
- **🎨 风格库**：导入小说 → AI 一次性分析写作风格（含长短段配比）→ 逐字段分段编辑 → 正文生成时套用
- **🧠 人格库**：导入作者访谈/随笔/创作谈 → AI 提取 5 维写作人格（情感强度/冲突深度/人情温度/语言人格/读者关系）→ 逐字段编辑 → 正文生成时注入
- **✍️ 写作工作台**：三栏布局（章节目录 + 正文编辑 + 大纲/细纲/校对/设定/伏笔/时间线）
  - 🌊 流式逐章生成：打字机效果，可随时中断
  - 📑 卷纲逐卷生成：每次生成一卷，基于前一卷上下文自然衔接，支持折叠查看
  - 📝 逐章细纲：基于卷纲和上一章细纲逐章生成
  - 🧹 去 AI 味：本地 50+ 禁用词扫描 + AI 改写
- **📖 事实簿**：工作台右侧设定面板，5 类事实（角色/设定/规则/关系/事件）+ 硬/软规则 → 生成时自动注入
